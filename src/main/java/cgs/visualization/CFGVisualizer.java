...
public class CFGVisualizer {
	...
	private static boolean onlyShowVirtualMethod=true;

	public static void main(String... args) {
		String name = "dummyMainMethod_com_example_chathook_ProcessMonitor.json";
		String file = "E:\\Git\\Github\\taintbench\\Tools\\FlowDroid_2.7.1_Release\\FD-2.7.1\\soot-infoflow-cmd\\"
				+ name;
	...
	private static void highLightEntryPoints(MultiGraph graph) {
		...
		for (Edge e : graph.getEdgeSet()) {
			...
			if (!incomingEdges.containsKey(source))
				incomingEdges.put(source, 0);
			if (!incomingEdges.containsKey(target))
				incomingEdges.put(target, 1);
			else
				incomingEdges.put(target, incomingEdges.get(target) + 1);
		}
