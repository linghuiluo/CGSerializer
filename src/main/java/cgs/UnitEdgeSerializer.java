...
public class UnitEdgeSerializer implements JsonSerializer<ExceptionalUnitGraph> {
	...
	public JsonElement serialize(ExceptionalUnitGraph graph, Type typeOfSrc, JsonSerializationContext context) {
	...
	public static int getID(Unit unit) {
		if (unitIDMap.containsKey(unit))
			return unitIDMap.get(unit);
		else {
			currentID++;
